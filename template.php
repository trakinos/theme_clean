...
// Add Zen Tabs styles.
if (theme_get_setting('theme_clean_tabs')) {
  drupal_add_css(drupal_get_path('theme', 'theme_clean') . '/css/tabs.css');
}

/**
 * Implements hook_preprocess_html().
 */
function theme_clean_preprocess_html(&$variables) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user).
  foreach ($user->roles as $role) {
    $variables['classes_array'][] = 'role-' . theme_clean_id_safe($role);
  }
  ...
  if (!$variables['is_front']) {
    ...
    $variables['classes_array'][] = theme_clean_id_safe('page-' . $path);
    $variables['classes_array'][] = theme_clean_id_safe('section-' . $section);
  ...
  // Add IE classes.
  if (theme_get_setting('theme_clean_ie_enabled')) {
    $theme_clean_ie_enabled_versions = theme_get_setting('theme_clean_ie_enabled_versions');
    if (in_array('ie8', $theme_clean_ie_enabled_versions, TRUE)) {
    ...
    if (in_array('ie9', $theme_clean_ie_enabled_versions, TRUE)) {
 ...
 */
function theme_clean_preprocess_page(&$variables, $hook) {
 ...
 */
function theme_clean_preprocess_node(&$variables) {
  ...
  // Merge first/last class (from theme_clean_preprocess_page) into classes array of
  // current node object.
  $node = $variables['node'];
 ...
 */
function theme_clean_preprocess_block(&$variables) {
 ...
 */
function theme_clean_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme_clean_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('theme_clean_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('theme_clean_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('theme_clean_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('theme_clean_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function theme_clean_id_safe($string) {
 ...
 */
function theme_clean_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = theme_clean_id_safe($element['#title']);
 ...
 */
function theme_clean_preprocess_menu_local_task(&$variables) {
 ...
 */
function theme_clean_menu_local_tasks(&$variables) {
  $output = '';
